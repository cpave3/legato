...
The TUI SHALL support starting and stopping the embedded web server via the `W` keyboard shortcut.

#### Scenario: Start web server from TUI
- **WHEN** the user presses `W` and the web server is not running
...
- **WHEN** the user presses `W` and the web server is running
