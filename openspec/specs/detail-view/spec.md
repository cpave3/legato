...
The detail view MUST display a status bar at the bottom showing available keybindings: `esc` back, `y` copy description, `Y` copy full context, `m` move, `o` open in browser.

#### Scenario: Status bar displays keybinding hints

- **WHEN** the detail view is rendered
- **THEN** the bottom status bar shows the keybinding hints: esc back, y copy description, Y copy full context, m move, o open in browser
