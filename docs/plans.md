...
```text
legato plan submit <directory> [--task ID] [--name NAME]
...
`LEGATO_TASK_ID` and `LEGATO_PLAN_NAME` are environment fallbacks. CLI mutations broadcast `plan_changed` over the existing IPC bus.
