...
func TestColumnHeaderContainsName(t *testing.T) {
	...
	out := RenderColumn("Doing", cards, 20, true, 0, testColIcons)
...
func TestColumnHeaderContainsCount(t *testing.T) {
	...
	out := RenderColumn("Ready", cards, 20, false, -1, testColIcons)
...
func TestColumnEmptyRendersHeader(t *testing.T) {
	out := RenderColumn("Backlog", nil, 20, false, -1, testColIcons)
...
func TestColumnRendersCards(t *testing.T) {
	...
	out := RenderColumn("Doing", cards, 30, true, 0, testColIcons)
...
func TestColumnActiveVsInactive(t *testing.T) {
	cards := []CardData{{Key: "REX-1", Title: "Test", Priority: "High", IssueType: "Bug"}}
	active := RenderColumn("Doing", cards, 30, true, 0, testColIcons)
	inactive := RenderColumn("Doing", cards, 30, false, -1, testColIcons)
	if active == "" || inactive == "" {
