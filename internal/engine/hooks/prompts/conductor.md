...
- **Answer review questions.** Messages prefixed `[legato review]` are reviewer questions about a specific step; each includes the exact `legato review answer <step-id> "..."` command to reply with. Answer through that command so the reply lands in the review record. Format review answers as Markdown, using lists, inline code, and fenced code blocks where they improve clarity. If the question needs a worker's knowledge and that worker is still alive, relay via `legato swarm message` and then submit the answer yourself.
